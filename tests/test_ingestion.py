...
from ingestion.parser import _extrair_campos, parse_xlsx


class TestParseXLSX:
    """Suite de testes para o parser de arquivo único."""
    ...
    def test_valor_ausente_nao_e_inventado_como_zero(self):
        linha = [None] * 16
        linha[0] = "000101 - PRODUTO ALFA"
        linha[7] = date(2025, 1, 15)
        linha[8] = "PROTOCOLO-1"
        linha[9] = "001-100001"

        registro = _extrair_campos(tuple(linha))

        assert registro is not None
        assert registro["valor_total_venda"] is None
...
class TestIngestDirectory:
    ...
    def test_diretorio_vazio_retorna_dataframe_vazio(self, diretorio_vazio):
        """Diretório sem arquivos XLSX retorna DataFrame vazio."""
        df = ingest_directory(str(diretorio_vazio))
        assert len(df) == 0

    def test_concatena_multiplos_arquivos(self, mapa_vendas_path: Path, tmp_path):
        """Dois arquivos são concatenados no mesmo DataFrame."""
        import shutil
        f1 = tmp_path / "a.xlsx"
        f2 = tmp_path / "b.xlsx"
        ...
        assert len(df) == 10  # 5 vendas x 2 arquivos

    def test_coluna_arquivo_origem_presente(self, mapa_vendas_path: Path, tmp_path):
        """Cada linha tem coluna 'arquivo_origem' com o nome do arquivo."""
        import shutil
        f1 = tmp_path / "a.xlsx"
        shutil.copy2(mapa_vendas_path, f1)
        df = ingest_directory(str(tmp_path))
    ...
    def test_coluna_arquivo_origem_distinta_por_arquivo(
        self, mapa_vendas_path: Path, tmp_path
    ):
        """Arquivos diferentes têm valores diferentes em arquivo_origem."""
        import shutil
    ...
    def test_cobertura_do_xlsx_e_derivada_das_vendas(
        self, mapa_vendas_path: Path, tmp_path
    ):
        """Sem período no cabeçalho, a cobertura vem das datas do próprio relatório."""
        import shutil

        shutil.copy2(mapa_vendas_path, tmp_path / "periodo.xlsx")

        df = ingest_directory(tmp_path)

        assert set(df["periodo_inicio"]) == {date(2025, 1, 15)}
        assert set(df["periodo_fim"]) == {date(2025, 3, 15)}
        assert set(df["confianca_metadados"]) == {"DERIVADA"}

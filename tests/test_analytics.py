...
class TestComputeKPIs:
        ...
        return pd.DataFrame([
            {
                ...
                "valor_total_venda_centavos": 15000,
                "quantidade": 2,
            },
            {
                ...
                "id_venda": "001001-1000012025-01-15",
                "data_emissao": date(2025, 1, 15),
                "valor_total_venda_centavos": 30000,
                "quantidade": 1,
            },
            {
                ...
                "valor_total_venda_centavos": 4500,
                "quantidade": 10,
            },
        ])

    def test_total_faturamento(self):
        """SUM de valor_total_venda_centavos."""
        kpis = compute_kpis(self._df_base())
        assert kpis["total_faturamento_centavos"] == 49500

    def test_total_monetario_permanece_inteiro(self):
        df = self._df_base()
        df["valor_total_venda_centavos"] = [10, 20, 30]

        kpis = compute_kpis(df)

        assert kpis["total_faturamento_centavos"] == 60
        assert isinstance(kpis["total_faturamento_centavos"], int)
    ...
    def test_ticket_medio(self):
        ...
        kpis = compute_kpis(self._df_base())
        assert kpis["ticket_medio_centavos"] == 24750  # 49500 / 2

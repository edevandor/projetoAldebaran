...
Lê arquivos com estrutura de blocos por artigo, extrai apenas as linhas
...
def _cell_num(val: Any) -> float | None:
    """Converte célula numérica sem confundir ausência ou erro com zero."""
    if val is None:
        return None
    try:
        return float(val)
...
def _extrair_campos(row: tuple) -> dict | None:
    ...
    return {
        ...
        "protocolo": _cell_str(_campo(COL_PROTOCOLO)),
        "valor_total_venda": _cell_num(_campo(COL_TOTAL_VENDA)),
        "quantidade": _cell_num(_campo(COL_QUANTIDADE)),
        "unidade_medida": _cell_str(_campo(COL_UNIDADE)),
        "sistema_origem": "ATUAL",
        "confianca_data": "Ausente" if data_emissao is None else "Registrada",
    }
...
def parse_xlsx(path: str | Path) -> list[dict]:
    ...
    ws = wb.active
    if ws is None:
        raise ValueError(f"Arquivo sem worksheet ativa: {path}")

"""Testes de exportação para inspecção (JSON/Markdown), auditoria e integração.

O contrato da camada de saída publicada está em test_export_schema.py.
"""

import json
from datetime import date
from pathlib import Path

import pandas as pd
import pytest

from export.formatters import export_to_json, export_to_markdown


class TestExportToJSON:
    """Suite de testes para exportação JSON."""

    def _df(self) -> pd.DataFrame:
        return pd.DataFrame([
            {
                "produto": "0000001 - FERRAMENTA A",
                "codigo_produto": "0000001",
                "data_emissao": date(2025, 1, 15),
                "valor_total_venda": 150.0,
                "quantidade": 2,
            }
        ])

    def test_cria_arquivo_json(self, tmp_path: Path):
        """export_to_json cria um arquivo JSON no caminho especificado."""
        path = tmp_path / "saida.json"
        resultado = export_to_json(self._df(), path)
        assert resultado.exists()
        assert resultado.suffix == ".json"

    def test_json_contem_registros(self, tmp_path: Path):
        """JSON gerado contém os registros do DataFrame."""
        path = tmp_path / "saida.json"
        export_to_json(self._df(), path)
        with open(path, encoding="utf-8") as f:
            dados = json.load(f)
        assert len(dados) == 1
        assert dados[0]["codigo_produto"] == "0000001"

    def test_data_convertida_para_string(self, tmp_path: Path):
        """Data é convertida para string ISO."""
        path = tmp_path / "saida.json"
        export_to_json(self._df(), path)
        with open(path, encoding="utf-8") as f:
            dados = json.load(f)
        assert isinstance(dados[0]["data_emissao"], str)
        assert "2025-01-15" in dados[0]["data_emissao"]

    def test_dataframe_vazio_cria_json_vazio(self, tmp_path: Path):
        """DataFrame vazio gera array JSON vazio."""
        df = pd.DataFrame(columns=["produto", "valor_total_venda"])
        path = tmp_path / "vazio.json"
        export_to_json(df, path)
        with open(path, encoding="utf-8") as f:
            dados = json.load(f)
        assert dados == []

    def test_cria_diretorio_intermediario(self, tmp_path: Path):
        """Cria diretórios intermédios se não existirem."""
        path = tmp_path / "sub" / "pasta" / "saida.json"
        export_to_json(self._df(), path)
        assert path.exists()


class TestExportToMarkdown:
    """Suite de testes para exportação Markdown."""

    def _kpis(self) -> dict:
        return {
            "total_faturamento_centavos": 1000000,
            "qtd_notas_fiscais": 15,
            "total_itens": 50,
            "ticket_medio_centavos": 66667,
            "top_produtos_valor": [
                {"codigo": "0001", "descricao": "PRODUTO TOP", "total_centavos": 500000},
                {"codigo": "0002", "descricao": "PRODUTO B", "total_centavos": 300000},
            ],
            "top_produtos_qtd": [
                {"codigo": "0003", "descricao": "UNIDADE", "total": 100.0},
            ],
            "vendas_por_mes": [
                {"mes": "2025-01", "total_centavos": 500000},
                {"mes": "2025-02", "total_centavos": 500000},
            ],
        }

    def test_cria_arquivo_markdown(self, tmp_path: Path):
        """export_to_markdown cria arquivo .md."""
        path = tmp_path / "relatorio.md"
        resultado = export_to_markdown(self._kpis(), path)
        assert resultado.exists()
        assert resultado.suffix == ".md"

    def test_markdown_tem_titulo(self, tmp_path: Path):
        """Arquivo MD começa com título."""
        path = tmp_path / "relatorio.md"
        export_to_markdown(self._kpis(), path)
        conteudo = path.read_text(encoding="utf-8")
        assert "# Relatório de KPIs" in conteudo

    def test_markdown_tem_metricas_globais(self, tmp_path: Path):
        """Métricas globais aparecem no markdown."""
        path = tmp_path / "relatorio.md"
        export_to_markdown(self._kpis(), path)
        conteudo = path.read_text(encoding="utf-8")
        assert "R$ 10_000.00" in conteudo
        assert "15" in conteudo

    def test_markdown_sem_top_produtos(self, tmp_path: Path):
        """KPIs sem top produtos não quebram a exportação."""
        kpis = {"total_faturamento_centavos": 0, "qtd_notas_fiscais": 0,
                "total_itens": 0, "ticket_medio_centavos": 0}
        path = tmp_path / "minimo.md"
        export_to_markdown(kpis, path)
        conteudo = path.read_text(encoding="utf-8")
        assert conteudo


class TestPipelineIntegration:
    """Testes de regressão — pipeline completo."""

    def test_pipeline_com_dados_sinteticos(self):
        """Pipeline completo funciona com dados sintéticos da fixture."""
        from analytics.kpis import compute_kpis
        from consolidation.consolidator import consolidate
        from ingestion.directory import ingest_directory
        from transformation.standardizer import standardize

        df = ingest_directory("tests/fixtures")
        assert len(df) > 0, "Deveria ter lido o arquivo de teste"
        df = standardize(df)
        assert "id_venda" in df.columns
        df, _ = consolidate(df)
        kpis = compute_kpis(df)
        assert kpis["total_faturamento_centavos"] > 0
        assert kpis["qtd_notas_fiscais"] > 0

    def test_pipeline_orquestrada(self):
        """run_pipeline executa todas as etapas."""
        import tempfile
        from pathlib import Path

        from pipeline import run_pipeline

        with tempfile.TemporaryDirectory() as tmp:
            tmp_p = Path(tmp)
            resultado = run_pipeline(
                "tests/fixtures",
                output_dir=tmp_p / "processed",
                csv_path=tmp_p / "dados.csv",
            )
            assert "dados" in resultado
            assert "kpis" in resultado
            assert "etapas" in resultado
            assert len(resultado["dados"]) > 0
            assert resultado["kpis"]["total_faturamento_centavos"] > 0

    def test_etapas_no_relatorio(self):
        """Relatório de etapas contém todas as fases."""
        import tempfile
        from pathlib import Path

        from pipeline import run_pipeline

        with tempfile.TemporaryDirectory() as tmp:
            tmp_p = Path(tmp)
            resultado = run_pipeline(
                "tests/fixtures",
                output_dir=tmp_p / "processed",
                csv_path=tmp_p / "dados.csv",
            )
            etapas = resultado["etapas"]
            for etapa in ["ingestao", "deduplicacao", "padronizacao", "versoes",
                          "validacao", "analytics", "parquet"]:
                assert etapa in etapas, f"Etapa {etapa} ausente"
                assert "duracao_s" in etapas[etapa], \
                    f"duração ausente em {etapa}"

    def test_export_apos_pipeline(self, tmp_path: Path):
        """Exportar JSON + MD após pipeline funciona."""
        import tempfile

        from export.formatters import export_to_json, export_to_markdown
        from pipeline import run_pipeline

        with tempfile.TemporaryDirectory() as tmp:
            tmp_p = Path(tmp)
            resultado = run_pipeline(
                "tests/fixtures",
                output_dir=tmp_p / "processed",
                csv_path=tmp_p / "dados.csv",
            )
            json_path = tmp_path / "dados.json"
            md_path = tmp_path / "kpis.md"
            export_to_json(resultado["dados"], json_path)
            export_to_markdown(resultado["kpis"], md_path)
            assert json_path.exists()
            assert "R$" in md_path.read_text(encoding="utf-8")


class TestAuditIntegration:
    """Testes para o módulo de auditoria."""

    def test_pipeline_run_criado(self):
        """criar_relatorio_pipeline produz PipelineRun."""
        from audit.auditor import criar_relatorio_pipeline

        etapas = {
            "ingestao": {"linhas": 100, "duracao_s": 2.5},
            "padronizacao": {"linhas": 100, "duracao_s": 0.3},
        }
        run = criar_relatorio_pipeline(etapas)
        assert run.sucesso
        assert run.duracao_s == 2.8
        assert "ingestao" in run.etapas

    def test_pipeline_run_com_erros(self):
        """PipelineRun com erros relata status de falha."""
        from audit.auditor import PipelineRun

        run = PipelineRun(erros=["Arquivo não encontrado"])
        assert not run.sucesso
        assert "✗" in run.resumo()

    def test_resumo_contem_etapas(self):
        """Resumo mostra número de etapas."""
        from audit.auditor import criar_relatorio_pipeline

        etapas = {"ingestao": {"linhas": 50, "duracao_s": 1.0}}
        run = criar_relatorio_pipeline(etapas)
        resumo = run.resumo()
        assert "1 etapas" in resumo
        assert "50 linhas" in resumo

    def test_audit_com_pipeline_real(self):
        """Auditoria integrada com pipeline real coleta métricas."""
        import tempfile

        from audit.auditor import criar_relatorio_pipeline
        from pipeline import run_pipeline

        with tempfile.TemporaryDirectory() as tmp:
            tmp_p = Path(tmp)
            resultado = run_pipeline(
                "tests/fixtures",
                output_dir=tmp_p / "processed",
                csv_path=tmp_p / "dados.csv",
            )
            run = criar_relatorio_pipeline(resultado["etapas"])
            assert run.sucesso
            assert run.duracao_s > 0
            assert len(run.etapas) >= 4


class TestValidacaoEAuditoriaNoPipeline:
    """A execução tem de deixar rasto: todo número publicado é rastreável."""

    def _executar(self, tmp_path: Path) -> dict:
        from pipeline import run_pipeline

        return run_pipeline("tests/fixtures", output_dir=tmp_path / "processed")

    def test_validacao_faz_parte_da_execucao(self, tmp_path: Path):
        resultado = self._executar(tmp_path)
        assert "validacao" in resultado
        assert "validacao" in resultado["etapas"]

    def test_auditoria_gravada_em_disco(self, tmp_path: Path):
        self._executar(tmp_path)
        assert (tmp_path / "processed" / "execucao.json").exists()

    def test_auditoria_registra_etapas_e_validacao(self, tmp_path: Path):
        resultado = self._executar(tmp_path)
        conteudo = json.loads(
            (tmp_path / "processed" / "execucao.json").read_text(encoding="utf-8")
        )
        assert conteudo["etapas"]
        assert "validacao" in conteudo
        assert "linhas" in conteudo["validacao"]
        assert conteudo["timestamp"]
        assert conteudo["id_execucao"] == resultado["run_id"]
        assert conteudo["versao_schema"]
        assert conteudo["versao_pipeline"]

    def test_id_execucao_liga_parquet_e_auditoria(self, tmp_path: Path):
        resultado = self._executar(tmp_path)
        conteudo = json.loads(
            (tmp_path / "processed" / "execucao.json").read_text(encoding="utf-8")
        )

        assert resultado["dados"]["id_execucao"].nunique() == 1
        assert resultado["dados"]["id_execucao"].iloc[0] == conteudo["id_execucao"]

    def test_auditoria_preserva_historico_e_fingerprints(self, tmp_path: Path):
        resultado = self._executar(tmp_path)
        conteudo = json.loads(
            (tmp_path / "processed" / "execucao.json").read_text(encoding="utf-8")
        )
        historico = (
            tmp_path / "processed" / "auditoria" / f"{resultado['run_id']}.json"
        )

        assert historico.exists()
        assert conteudo["entradas"]
        assert all(len(item["sha256"]) == 64 for item in conteudo["entradas"])
        assert conteudo["saidas"][0]["arquivo"] == "vendas_consolidadas.parquet"
        assert len(conteudo["saidas"][0]["sha256"]) == 64
        assert conteudo["status"] == "SUCESSO"
        assert conteudo["revisao_codigo"]

    def test_execucao_fatal_preserva_auditoria_de_falha(self, tmp_path: Path):
        from pipeline import PipelineContractError, run_pipeline

        origem = tmp_path / "raw"
        origem.mkdir()

        with pytest.raises(PipelineContractError, match="Nenhum registro"):
            run_pipeline(
                origem,
                output_dir=tmp_path / "processed",
                legado_dir=None,
            )

        falhas = list((tmp_path / "processed" / "auditoria").glob("*-falha.json"))
        assert len(falhas) == 1
        conteudo = json.loads(falhas[0].read_text(encoding="utf-8"))
        assert conteudo["status"] == "FALHA"
        assert conteudo["erros"]
        assert not (tmp_path / "processed" / "execucao.json").exists()

    def test_parquet_publicado_por_omissao(self, tmp_path: Path):
        self._executar(tmp_path)
        assert (tmp_path / "processed" / "vendas_consolidadas.parquet").exists()

    def test_falha_tardia_nao_substitui_parquet_anterior(
        self, tmp_path: Path, monkeypatch
    ):
        from pipeline import run_pipeline

        self._executar(tmp_path)
        parquet = tmp_path / "processed" / "vendas_consolidadas.parquet"
        conteudo_anterior = parquet.read_bytes()

        def falhar_auditoria(*args, **kwargs):
            raise OSError("falha simulada na auditoria")

        monkeypatch.setattr("pipeline._gravar_auditoria", falhar_auditoria)

        with pytest.raises(OSError, match="falha simulada"):
            run_pipeline("tests/fixtures", output_dir=tmp_path / "processed")

        assert parquet.read_bytes() == conteudo_anterior

    def test_falha_durante_promocao_restaura_publicacao_anterior(
        self, tmp_path: Path, monkeypatch
    ):
        import pandas as pd

        from pipeline import run_pipeline

        primeiro = self._executar(tmp_path)
        destino_auditoria = tmp_path / "processed" / "execucao.json"
        replace_original = Path.replace

        def falhar_no_marcador(source: Path, target: Path):
            if source.name == "execucao.json" and Path(target) == destino_auditoria:
                raise OSError("falha simulada na promoção")
            return replace_original(source, target)

        monkeypatch.setattr(Path, "replace", falhar_no_marcador)

        with pytest.raises(OSError, match="falha simulada"):
            run_pipeline("tests/fixtures", output_dir=tmp_path / "processed")

        parquet = pd.read_parquet(
            tmp_path / "processed" / "vendas_consolidadas.parquet"
        )
        auditoria = json.loads(destino_auditoria.read_text(encoding="utf-8"))
        assert set(parquet["id_execucao"]) == {primeiro["run_id"]}
        assert auditoria["id_execucao"] == primeiro["run_id"]


class TestResumoDaExecucao:
    """O resumo conta o dado publicado, não a soma das etapas."""

    def test_nao_soma_linhas_repetidas_entre_etapas(self):
        from audit.auditor import criar_relatorio_pipeline

        etapas = {
            "ingestao": {"linhas": 277, "duracao_s": 0.1},
            "padronizacao": {"linhas": 277, "duracao_s": 0.1},
            "consolidacao": {"linhas": 270, "duracao_s": 0.1},
        }
        run = criar_relatorio_pipeline(etapas)
        assert run.linhas_publicadas() == 270
        assert "270 linhas" in run.resumo()

    def test_milhar_no_formato_brasileiro(self):
        from audit.auditor import criar_relatorio_pipeline

        run = criar_relatorio_pipeline({"ingestao": {"linhas": 149129, "duracao_s": 1.0}})
        assert "149.129 linhas" in run.resumo()
